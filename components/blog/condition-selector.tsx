...
export function ConditionSelector({ form }: { form: UseFormReturn<BlogFormValues> }) {
  ...
  useEffect(() => {
    axios
      .get<Condition[]>("https://web.pharmaplus.co.ke/ecmws/read_conditions_info/fetch", {
        headers: {
                      ...
                      {filteredConditions.length > 0 ? (
                        filteredConditions.map((condition, index) => (
                          <div
                            ...
                            onClick={() => handleSelectCondition(condition.title)}
                          >
                            <div className="font-medium">{condition.title}</div>
                          </div>
                        ))
                      ) : (
                        <div className="p-2 text-center text-muted-foreground">No conditions found</div>
                      )}
                    </ScrollArea>
                  </div>
                )}
